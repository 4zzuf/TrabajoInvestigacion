...
def _simular_silencioso(numero_autobuses):
...
def tiempo_promedio_para_autobuses(numero_autobuses):
    """Calcula el tiempo promedio de intercambio operativo por autobús en minutos."""
    estacion = _simular_silencioso(numero_autobuses)
    if estacion.intercambios_operativos == 0:
        return 0
    tiempo_total = estacion.tiempo_espera_total + estacion.intercambios_operativos * TIEMPO_REEMPLAZO
    return (tiempo_total / estacion.intercambios_operativos) * 60
...
def graficar_tiempos_intercambio(block=True):
    ...
    plt.ylabel("Tiempo promedio de intercambio operativo (min)")
